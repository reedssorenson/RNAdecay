...
modOptimization = function(gene,
  ...
  try(if (sum(grepl(paste0("_", nTreat, "sse"), names(getLoadedDLLs()))) != 2)
    stop(
      "high performance objective functions need to be loaded as compilied dynamically linked libraries (.dll or .so) before modeling. You can check which are loaded using 'getLoadedDLLs()'",
      call. = FALSE
    ))
  ...
  results4a <-
    sapply(models[models %in% rownames(mod)[mod$b == max(mod$b)]], function(x,
      ...
      AICc_est = fun_aicc(LL.fun(
        ...
        y = sig.fun(x = obj.Exp$fn(par.est[1:nTreat]),
                    n = nSet),
        n = nSet
      ),
      p = nPar,
