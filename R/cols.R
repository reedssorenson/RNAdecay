...
#' @param w,x,y,z character string or regular expression passed to grep pattern argument
#'
#' @return  returns a vector of integer indices of the column names of \code{df} that match to all of \code{w}, \code{x}, \code{y}, and \code{z}
#'
#' @export
#'
#' @examples
...
cols = function(df,
  ...
  if (is.na(x))
    ...
    intersect(intersect(grep(w, names(df)), grep(x, names(df))), grep(y, names(df)))
  else
    intersect(intersect(intersect(grep(w, names(
      df
    )), grep(x, names(
      df
    ))), grep(y, names(df))), grep(z, names(df)))
}
