...
#'
#' The modOptimization function finds the estimates of model parameters by
...
#'     these 50 starting parameter sets using pre-compiled C++ functions
#'     loaded from dynamically linked libraries stored in the package on all
#'     models specified in the models argument.
...
#' @param file.only logical; should output only be written to file (TRUE) or
...
#' @export
#'
#' @return returns (if \code{file.only = FALSE}) and writes to \code{path} a
#'     data frame of model optimization results for \code{models} one row for
#'     each for
#' \code{gene} using values for it found in \code{data}, the columns of the
#'     data frame are:  geneID, mod (model), model estimates [alpha_treatment1,
#'     ..., alpha_treatmentn, beta_treatment1, ..., beta_treatmentn, sigma2],
#'     logLik (maximum log likelihood), nPar (number of parameters in the
#'     model), nStarts (number of parameter starting value sets (of 50) that
#'     converged on a maximum likelihood peak), J (number of parameter starting
#'     value sets that converged on the highest - within 1e-4 - maximum
#'     likelihood of all parameter starting value sets), range.LL (range of
#'     maximum likelihoods values reached by algorithm convergence from all
#'     parameter starting value sets), nUnique.LL (number of unique maximum
#'     likelihoods values reached by algorithm convergence from all parameter
#'     starting value sets), C.alpha (sum of all coefficients of variation for
#'     each column of alpha estimates), C.beta (sum of all coefficients of
#'     variation for each column of beta estimates), C.tot (C.alpha+C.beta),
#'     AICc (calculated from the single highest maximum likelihood of all
#'     parameter starting value sets), AICc_est (calculated from the log
#'     likelihood value computed by using the mean of each parameter from all
#'     optimizations that converged on the highest maximum likelihood of all
#'     starting parameter value sets.)
#'
#' @examples
#'
...
#'                             0.8679866, 0.6798788, 0.2683555, 0.5120951,
#'                             0.2593122, 1.1348219, 0.8535835, 0.6423996,
#'                             0.5308946, 0.4592902, 1.1104068, 0.5966838,
#'                             0.3949790, 0.3742632, 0.2613560)),
...
#'                           c('mod1','mod2','mod3')))),
#'                 mod = as.data.frame(
#'                 t(matrix(c(1,1,1,2,1,3,2,1,2,2,2,3), nrow=2,
#'                         dimnames=
#'                         list(c('a','b'),paste0('mod',seq_len(6)))))),
#'                 file.only = FALSE)
#'



modOptimization = function(gene,
                           data,
                           alpha.bounds,
                           beta.bounds,
                           models,
  ...
  genoSet = seq_len(length(unique(data$rep)) * length(unique(data$t.decay)))
  nTreat = length(unique(data$treatment))
  try(if (sum(grepl(paste0("_", nTreat, "sse"), names(getLoadedDLLs()))) != 2)
    stop(
      "high performance objective functions need to be loaded as compilied
      dynamically linked libraries (.dll or .so) before modeling. You can
      check which are loaded using 'getLoadedDLLs()'",
      call. = FALSE
    ))
  nSet = length(genoSet) * nTreat

  try(if (nTreat > 4)
    stop("modOptimization can only handle up to 4 treatments."))
  ...
  A <- seq(0, alpha.bounds[2], by = 0.001)
  ### Define grid for evaluating model 239 while determining starting points.
  a <- seq(alpha.bounds[1], alpha.bounds[2], by = 0.001)
  b <- seq(beta.bounds[1], beta.bounds[2], by = 0.001)

  ### Pick 25 points near the 'peak' of null double exponential model.
  ### Find the SSE surface for the double exponential model...
  sse.nulldExp <- sapply(b, function(x) {
    sapply(
      a,
      FUN = sse.nulldExp.fun,
      b = x,
      m = m,
      t = t
    )
  ...
  ### Pick 25 points near the 'peak' of Model 240. Find the SSE surface for
  ### model 240...
  ...
  bX0 <-
    matrix(c(
      ...
      stats::runif(25 * nTreat, beta.bounds[1], beta.bounds[2])
    ), ncol = nTreat)
  ...
  X0 <- cbind(aX0, bX0)
  colnames(X0) <-
    c(paste0("alpha.int", seq_len(nTreat)), paste0("beta.int", seq_len(nTreat)))

  # set default parameters
  par.default <- c(rep(0, nTreat), rep(1, nTreat))
  names(par.default) <-
    c(paste0("a", seq_len(nTreat)), paste0("b", seq_len(nTreat)))


  # OPTIMIZATION OF THE DOUBLE EXPONENTIAL MODELS

  #### Create the objective function in R. ### This has to be done for each
  #### gene!
  ...
  results4ab <-
    sapply(models[models %in% rownames(mod)[mod$b != max(mod$b)]],
           function(x,
                    eps,
                    nSet,
                    md,
        ...
        fit <-
          nloptr::slsqp(
            x0 = as.numeric(starts[seq_len(ncol(X0))]),
            fn = obj.dExp$fn,
            gr = obj.dExp$gr,
            heq = const[[x]],
            lower = c(rep(alpha.bounds[1],
                          nTreat), rep(beta.bounds[1], nTreat)),
            upper = c(rep(alpha.bounds[2],
                          nTreat), rep(beta.bounds[2], nTreat))
          )
        unlist(c(
          geneID = gene,
          mod = x,
      ...
      AICc_est = fun_aicc(LL.fun(
        x = obj.dExp$fn(par.est),
        y = sig.fun(x = obj.dExp$fn(par.est),
                    n = nSet),
        n = nSet
      ),
      p = nPar,
      ...
      return(fit)
    }, eps = eps, nSet = nSet, grp = group, md = mod,
    alpha.bounds = alpha.bounds,
    beta.bounds = beta.bounds)
  ...
  X0 <- aX0
  colnames(X0) <- c(paste0("alpha.int", seq_len(nTreat)))

  obj.Exp = TMB::MakeADFun(
    data = list(t = t, m = m),
    parameters = par.default[seq_len(nTreat)],
    silent = TRUE,
    ...
    sapply(models[models %in% rownames(mod)[mod$b == max(mod$b)]],
        ...
        fit <-
          nloptr::slsqp(
            ...
            gr = obj.Exp$gr,
            heq = const[[x]],
            lower = rep(alpha.bounds[1], nTreat),
            upper = rep(alpha.bounds[2], nTreat)
          )
        unlist(c(
          geneID = gene,
          mod = x,
      ...
      AICc = fun_aicc(max.LL, nPar, nSet)
      AICc_est = fun_aicc(LL.fun(
        x = obj.Exp$fn(par.est[seq_len(nTreat)]),
        y = sig.fun(x = obj.Exp$fn(par.est[seq_len(nTreat)]),
                    n = nSet),
        n = nSet
      ),
      p = nPar,
  ...
  utils::write.table(results, paste0(path, "/", gene, "_results.txt"),
                     sep = "\t")
  cat(gene, "done     \n")  #; utils::timestamp()
