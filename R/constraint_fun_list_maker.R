...
#'
#' @return  Returns a list of constraint functions to be passed to the optimization function.
#'
#' @export
#'
#' @examples
...
constraint_fun_list_maker= function(mods, groups) {
  ...
  # function to make a function with an extra argument "body.subset" so select lines of the function body by numerical index
  make_function <- function(body.subset, args, body, last.line = nrow(groups), env = parent.frame()) {
    f <- function() {}
    formals(f) <- args
  ...
  if(ncol(groups) == 4){
    const_Ind=list(c(NULL),c(4,5),c(2,3),c(1,3),c(1,2),6,5,4,3,2,1,c(1,6),c(2,5),c(3,4),c(1,2,3),NULL)
    names(const_Ind)=paste0("gp",1:16)
    const_Ind=unlist(lapply(const_Ind[1:15],function(x) lapply(lapply(const_Ind,function(y) y+6),function(z) c(x,z))),recursive = FALSE)
    names(const_Ind)=paste0("mod",1:240)
    constraint_fun_list=lapply(const_Ind, function(x) {make_function(body.subset=x,
                                                                     args = alist(pars=),
  ...
  if(ncol(groups) == 3) {
    ...
    const_Ind=unlist(lapply(const_Ind[1:5],function(x) lapply(lapply(const_Ind,function(y) y+3),function(z) c(x,z))),recursive = FALSE)
  ...
  if(ncol(groups) == 2) {
    ...
    const_Ind=unlist(lapply(const_Ind[1:2],function(x) lapply(lapply(const_Ind,function(y) y+1),function(z) c(x,z))),recursive = FALSE)
  ...
  if(ncol(groups) == 1) {
    ...
    names(const_Ind)=paste0("gp",1)
    const_Ind=unlist(lapply(const_Ind[1],function(x) lapply(lapply(const_Ind,function(y) y),function(z) c(x,z))),recursive = FALSE)
